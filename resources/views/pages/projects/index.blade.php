                ...
                <div class="col-md-4">
                    <div class="card h-100">
                        <div class="card-header {{ $colors[$status] }} text-white">
                            <h3>{{ $label }} <button class="btn btn-light btn-sm new-project"
                                    data-status="{{ $status }}">
                                    <i class="fas fa-plus"></i>
                                </button></h3>

                        </div>
                        <ul class="list-group list-group-flush bg-light" data-status="{{ $status }}"
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center"
                                    ...
                                    <div>
                                        <button class="btn btn-danger btn-sm delete-project"
                                            data-project-id="{{ $project->id }}">Eliminar</button>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
                ...
                <div class="modal-body">
                    <form id="commentForm">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id = "project-task-desc" name="project-task-desc"
                                required>
                        </div>
                        <div class="form-group
                    ">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="commentForm">
                        <div class="form-group">
                            <label for="comment">Comentario</label>
                            <input type="text" class="form-control" id = "edit-project-comment" name="comment"
                                required>
                        </div>
        ...
        function loadProjectTasks(projectId) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var paidAmount = response.invoices && response.invoices.paid && response.invoices.paid
                        .length > 0 ? response.invoices.paid[0].amount : 0;
                    var pendingAmount = response.invoices && response.invoices.pending && response.invoices
                        .pending.length > 0 ? response.invoices.pending[0].amount : 0;
                    var totalAmount = parseFloat(paidAmount) + parseFloat(pendingAmount);
                    var totalInvoices = response.invoices && response.invoices.total > 0 ? response.invoices
                        .total : 0;
